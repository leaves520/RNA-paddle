...
def load_train_data():
    assert os.path.exists("mydata/train.txt")
    assert os.path.exists("mydata/dev.txt")
    train = read_data("mydata/train.txt")
    dev = read_data("mydata/dev.txt")
    return train, dev


def load_test_data(name):
    if name == "a":
        assert os.path.exists("mydata/test_nolabel.txt")
        test = read_data("mydata/test_nolabel.txt", test=True)
    elif name == "b":
        assert os.path.exists("mydata/B_board_112_seqs.txt")
        test = read_data("mydata/B_board_112_seqs.txt", test=True)
    else:
...
def load_test_label_data():
    assert os.path.exists("mydata/test.txt")
    test = read_data("mydata/test.txt")
    return test
...
class MyDataset(Dataset):
    def __init__(self, data_label, data_type, seq_vocab, bracket_vocab, n_gram, process=False, graph=False, bond=False):
        ...
        self.n_gram = n_gram
        if not os.path.exists('mydata/ready_data'):
            os.makedirs('mydata/ready_data')
        if not graph:
            data_path = os.path.join('mydata/ready_data', f"{n_gram}_gram_{data_type}.pkl")
        else:
            data_path = os.path.join('mydata/ready_data', f"{data_type}_{bond}_graph.pkl")
...
class NormCollator:
    ...
    def __call__(self, examples):
        ...
        dots_padding = np.zeros((len(dots), max_seq_len), dtype=np.int32)

        for i, seq in enumerate(seqs):
            seqs_padding[i, :len(seq)] = seq
        for i, dot in enumerate(dots):
            dots_padding[i, :len(dot)] = dot
...
if __name__ == '__main__':
    ...
    dataset = PretrainingDataset(input_file='mydata/pretrain_mydata/test.pkl', max_pred_length=100)
    ...
    df = pd.read_csv('/code/mydata/rna/RNA_1500_valid_seq_dot.csv')
    df = df[:10]
    input_seq_ids[0]
